...
Le \gls{LLM} constitue l'intermédiaire entre les données et l'utilisateur. Basé sur du \gls{DPL}, il mobilise les technologies du \gls{NLP} et les combine a une faculté de raisonnement. De cette manière, il est capable d'analyser et d'extraire la sémantique d'une phrase mais aussi de formuler ses propres phrases. Dans ce processus, il intervient au moment de la réponse afin de converser avec l'utilisateur. Son fonctionnement est le suivant : lorsqu'il reçoit une requête il en extrait des \glspl{tk}, c'est-à-dire des mots. Il procède à une \gls{Vecsion} : chaque morceau de texte devient un \gls{vcs}, une suite de chiffres uniques. Il applique ensuite les différents outils issus du \gls{NLP} dont il dispose afin d'extraire du sens et des relations depuis ces \gls{vcs}. De cette manière, il \guillemet{comprend} la requête et peut ensuite répondre.
...
Deux approches permettent de combler cet écart : les \textit{frameworks} spécialisés comme VANNA AI et les \gls{AGSQL} comme LangChain\footcite{TexttoSQLInterrogerVos2026}. La première approche consiste à créer un processus de \gls{RAG} avec le schéma SQL, des exemples de requêtes et de la documentation professionnelle. Ainsi, n'importe quel \gls{LLM} peut s'entraîner sur ces données afin de comprendre son environnement. Il atteint vite ses limites car chaque changement d'architecture de la base amène à entraîner de nouveau le modèle et à mettre à jour la documentation. La seconde approche consiste en la spécialisation d'un ou plusieurs \gls{LLM} pour la compréhension et la génération de requête SQL. Ils sont dès lors capables de traiter des bases plus complexes grâce à une compréhension native du SQL. C'est cependant un outil plus complexe à mettre en place et énergivore demandant plus de compétences techniques.
...
Il faut chercher la solution dans un autre outil d'\gls{IA} : le \guillemet{chatbot}\newline

Les récentes évolutions technologiques en matière d'\gls{IA} ont amené des transformations terminologiques. Aujourd'hui, le terme \gls{chatbot} porte un sens confus que nous devons clarifier.

Le premier \guillemet{\gls{chatbot}} voit le jour en 1966 : c'est le programme ELIZA \footcite{adamopoulouChatbotsHistoryTechnology2020}. Il interagit avec l'utilisateur en transformant les phrases qui lui sont données en question. Il s'appuie sur les premières technologies de \gls{NLP} alors basées sur l'analyse computationnelle du langage naturel.\footnote{voir page \fullref{nlp}} Cette expérience démontre la capacité à transformer une phrase en \gls{vcs} permettant à l'ordinateur d'interagir avec le langage naturel. \footcite{adamopoulouChatbotsHistoryTechnology2020}.

L'apparition du mot \gls{chatbot} date de 1994 avec le programme \textit{Julia}. Il interagit avec l'utilisateur en répondant avec des phrases. Il repose sur l'accumulation de règles aux réponses prédéterminées à utiliser selon le cas. Il analyse la phrase, détermine à quel ensemble de règles elle se rattache et renvoie les réponses prédéterminées. C'est ce que l'on appelle l'\gls{IA} symbolique : il n'y a pas encore de \guillemet{réflexion}. Mais c'est cela que le mot \gls{chatbot} représente. 
...
L'arrivée des \gls{IA} à base de neurones artificiels et de \gls{DPL}  change le paysage du \gls{chatbot}. Sans règles, le programme est capable de réfléchir et de s'exprimer en autonomie ce que les \guillemet{\glspl{chatbot}} ne pouvaient pas faire. \footcite{adamopoulouChatbotsHistoryTechnology2020}
Leur capacité de génération et de compréhension ne faisant que grandir, les \guillemet{\gls{chatbot}} traditionnels ont peu à peu disparu.

C'est l'explosion public des \glspl{LLM} en 2020 a brouillé la barrière sémantique. Les \glspl{mod} comme ChatGPT peuvent comprendre une requête, générer des réponses et mémoriser des conversations entières. Très différents d'un \gls{chatbot}, ils en adoptent pourtant la même forme dans ce que l'on appel les \glspl{ac}. Cela vient mélanger les réalités technologiques : dans la position d'un utilisateur, ce sont les mêmes outils visuellement. Ainsi, le terme \guillemet{\gls{chatbot}} finit par désigner les deux outils.
...
Au sein de SkinSoft, la discussion autour de l'intégration d'un \gls{chatbot} dans le logiciel a fait ressortir cette évidente confusion. Lorsque l'on élargit le spectre à des projets et logiciels concurrents ayant implémenté un \guillemet{\gls{chatbot}} on se rend compte de la même imprécisions. 

Ainsi, dans ce présent mémoire lorsque nous parlons de \guillemet{\gls{chatbot}} nous désignons en réalité un \gls{ac}. Cela dans une volonté de s'inscrire dans une tendance terminologique actuelle et dominante. \newline
...
La combinaison entre le \gls{RAG} et la capacité conversationnelle d'un \gls{chatbot} ouvre d'autres perspectives. Ce système a déjà été appliqué à l'exploration de corpus documentaires afin d'accompagner les chercheurs dans leurs explorations. \footcite{tuzaInterrogerMemoireDentreprise2025} Cela permet des approches approfondies de corpus importants plus rapidement grâce aux capacités de conversation et de réflexion des \glspl{chatbot} limités par un \gls{RAG} aux seuls corpus. Appliquer ce principe à de la documentations métier et d'usage d'une application permet d'envisager un \gls{chatbot} capable d'accompagner l'utilisateur.\newline

Cela pose plusieurs questions. D'abord au sujet du choix de la documentations à propos des collections traitées. Pour exploiter pleinement le \gls{RAG} la documentation doit être limitée et uniforme. Cependant, la documentation visée est massive et hétérogènes. Une institution gérant des fonds très diversifiés peut donc rencontrer une première limite. Mais aussi sur le choix de la documentation propre au fonctionnement de l'application. Dans le cas de SkinSoft, il faut pouvoir différencier ce qui relève des fonctionnalités communes de celles propres aux modules adaptés à chaque typologies.

Ces deux points viennent souligner une limite du \gls{RAG} : sa rigidité. Ce processus recherche toujours dans la même base de données et donc dans des objectifs toujours similaire. L'hétérogénéité des collections représentées, la variétés des institutions utilisant ce logiciel et des pratiques entrent en contradiction avec ce principe.\newline

Sans projets ni études à l’appui, il est difficile de faire un état de l'art de cette emploi, encore peu répandue, de cette technologie. Cependant, notre rapide examen permet de conclure que cette perspective est encore expérimentale et semble rencontrer bien des soucis de définitions.
