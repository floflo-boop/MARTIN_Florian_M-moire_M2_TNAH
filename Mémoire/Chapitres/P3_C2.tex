...
Finalement, le processus d'indexation semi-automatisée soulève des enjeux relatifs aux outils de productions assistés par \gls{IA}. De manière générale, mais aussi propre au domaine de la gestion des collections. Mise à part des enjeux techniques et des questions d'\gls{ux} et d'\gls{ui}, ce processus remet en question des principes fondamentaux.\newline

D'abord le principe de rigueur scientifique. Contrairement à la \guillemet{recherche augmentée} ou au \gls{chatbot}, l'indexation semi-automatisée génère de la données qui doit avoir une valeur scientifique. Ce processus s'appui sur des formes d'\gls{IAG} : les \gls{LLM}, \gls{VLM} et \gls{ALM}. La génération des données par ces outils peut être compromise par des hallucinations impactant la qualité des données et donc leurs véracités scientifiques.

Limiter l'hallucination est encore un domaine d'amélioration. Nous avons vus que l'architecture \gls{RAG} est la plus fiable dans ce domaine, sans être infaillible, mais elle ne semble pas applicable ici. Finalement, cela contraint à adopter des architectures \textit{human in the loop} assez lourde, pouvant compromettre le gain de temps que l'on cherche à obtenir. Cela demande de nouveau procédé afin de certifier une certaine véracité scientifique.\newline

Ensuite, le principe de confiance des usagers. Fragiliser la véracité des informations des bases de données institutionnels amène à provoquer des doutes dans l'usage de ces applications. Les utilisateurs risquent de ne pas se servir de ce type d'outil, ou bien manière partielles par manque de confiance. 

On entre ici dans un domaine flous, à la croisée entre le procédé techniques et les questions d'\gls{ux} et d'\gls{ui}. Habituellement, ces deux domaines permettent de rassurer l'utilisateurs dans son usage d'une fonctionnalité. Mais devant la complexité de ces processus et les risques d'hallucinations, finalement cela ne semble plus être suffisant. Cela demande de nouveaux moyens.\newline

Il se pose également la question de la provenance des données indexées, qui revient à aborder des points éthiques et juridiques. Les institutions conservatrices doivent pouvoir retracer la provenance des données conservées en indiquant où la trouver. On entre dans une limite juridique et intellectuel : doit-on préciser que cela à été extrait par \gls{IA} ? Ou doit-on simplement pousser l'\gls{IA} à citer ces sources ? Comme nous l'avons vu,\footnote{Voir \fullref{juridique}} le cadre juridique impose de préciser quand un contenu à été généré par \gls{IA}. Mais lorsqu'il s'agit d'une données générées par l'humain mais redistribuée par l'\gls{IA}, devons-nous le préciser ?  

De manière plus approfondie, la mise en place d'un tel outil revient à définir profondément les pratiques des métiers en rapports avec la conservation de cette typologie documentaire. L'utilisateur du \gls{sgc} n'est plus créateur de la données, mais vérificateurs et validateurs. Si cela peut permettre d'accélérer le traitement massif des données, cela peut causer de la fatigue à force de répéter les mêmes opérations en boucles.\newline

Finalement, cet outil présente une triple complexité : technologique, d'usage et éthique. Nous avons encore du mal aujourd'hui à cerner et définir l'implications et l'impacts de tels outils dans le domaine de la gestion du patrimoine.
